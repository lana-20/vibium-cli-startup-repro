...
python3 verify_readme.py  # this README vs the repo, 32 checks
...
`EVIDENCE.md` is the block-by-block fact-check record — which of this README's 52
blocks carry checkable assertions, what checks them, and which rest on things this
repo cannot verify (a third party's comment, Firefox's BiDi behaviour). It is
generated by `record_evidence.py`, not hand-written, for the same reason the rest
of this is checked rather than asserted.
...
named check, and each was verified to fail when the defect is reintroduced. It
also checks its own claim about how many checks it runs, since that number drifts
